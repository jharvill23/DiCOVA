...
class Solver(object):
    ...
    def __init__(self):
        ...
        if not os.path.exists('coswara_partition.pkl'):
            utils.get_coswara_partition()
        ...
        if TRAIN:  # only copy these when running a training session, not eval session
            ...
            shutil.copy(src='coswara_partition.pkl', dst=os.path.join(exp_dir, 'coswara_partition.pkl'))
            self.partition = os.path.join(exp_dir, 'coswara_partition.pkl')
            # copy config as well
